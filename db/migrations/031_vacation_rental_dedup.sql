-- Aggregate Vacation Rental stats by unified customer_id, not raw hcp_customer_id.
--
-- A contact merged in HCP keeps its old customer id on jobs synced before the
-- merge (e.g. Nikola Radu = one HCP contact but two hcp_customer_ids, both
-- matched to one core.customer by phone). Grouping the airbnb branch by
-- hcp_customer_id double-counted such customers. Group by customer_id so every
-- unified contact is one row with combined jobs/revenue, and exclude airbnb
-- customers from the cadence branch by customer_id as well.
...
airbnb_customers as (
  ...
  from raw.hcp_jobs j
  join marts._hcp_to_customer l on l.hcp_customer_id = j.hcp_customer_id
  where j.raw_json->>'description' ilike any (array['%vacation rental%', '%airbnb%'])
),
airbnb_stats as (
  select
    l.customer_id,
    min(j.hcp_customer_id) as hcp_customer_id,
  ...
  from raw.hcp_jobs j
  join marts._hcp_to_customer l on l.hcp_customer_id = j.hcp_customer_id
  join airbnb_customers ac on ac.customer_id = l.customer_id
...
airbnb_cleaner as (
  ...
  order by l.customer_id, (j.raw_json->'work_timestamps'->>'completed_at')::timestamptz desc nulls last
),
recurring as (
  select j.hcp_customer_id, j.scheduled_start, j.created_at,
    round(j.total_amount / 100.0, 2) as visit_price, j.raw_json->>'recurrence_rule' as rrule
  from raw.hcp_jobs j where coalesce(j.raw_json->>'recurrence_rule', '') <> ''
),
completed as (
...
calc as (
  select *,
    case freq when 'WEEKLY' then 7 * intvl when 'DAILY' then 1 * intvl
...
cadence as (
  ...
  where c.period_days is not null
    and l.customer_id not in (select customer_id from airbnb_customers)
),
airbnb as (
  select
    s.customer_id, s.hcp_customer_id, 'Vacation Rental'::text as service_bucket, null::numeric as period_days,
  ...
  from airbnb_stats s
  left join airbnb_cleaner ac on ac.customer_id = s.customer_id
